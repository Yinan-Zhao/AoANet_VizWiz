...
# ./tools/generate_tsv.py --gpu 1,2 --cfg experiments/cfgs/faster_rcnn_end2end_resnet.yml --def models/vg/ResNet-101/faster_rcnn_end2end_final/test.prototxt --out tsv/VizWiz_resnet101_faster_rcnn_genome_trainval.tsv --net data/faster_rcnn_models/resnet101_faster_rcnn_final.caffemodel --split VizWiz_trainval
    ...
    if split_name == 'coco_test2014':
      ...
      with open('/data/visualgenome/image_data.json') as f:
        for item in json.load(f):
          ...
          split.append((filepath,image_id)) 
    elif split_name == 'VizWiz_trainval':
      
      with open('./annotations/train.json') as f:
        data = json.load(f)
        for item in data['images']:
          image_id = int(item['id'])
          filepath = os.path.join('./VizWiz_images/train/', item['file_name'])
          split.append((filepath,image_id))
      ...
      with open('./annotations/val.json') as f:
        ...
        for item in data['images']:
          image_id = int(item['id'])
          filepath = os.path.join('./VizWiz_images/val/', item['file_name'])
          split.append((filepath,image_id)) 
      ...
      with open('./annotations/test.json') as f:
        ...
        for item in data['images']:
          image_id = int(item['id'])
          filepath = os.path.join('./VizWiz_images/test/', item['file_name'])
          split.append((filepath,image_id))
        #split[0] = ('/home/yz9244/Up-Down-Captioner/data/VizWiz/images/VizWiz_test_000000021364.jpg', 21364)
    else:
...
#corrupt_list = ['/home/yz9244/Up-Down-Captioner/data/VizWiz/images/VizWiz_v2_000000037093.jpg']
corrupt_list = ['./VizWiz_images/train/VizWiz_train_00022628.jpg']
...
def generate_tsv(gpu_id, prototxt, weights, image_ids, outfile):
    ...
    if len(missing) > 0:
        ...
        with open(outfile, 'ab') as tsvfile:
            ...
            _t = {'misc' : Timer()}
            count = 0
            #image_ids[0] = ('/home/yz9244/Up-Down-Captioner/data/VizWiz/images/VizWiz_test_000000021364.jpg', 21364)
            for im_file,image_id in image_ids:
                print im_file
                if int(image_id) in missing and im_file not in corrupt_list:
                    _t['misc'].tic()
                    tmp = get_detections_from_im(net, im_file, image_id)
                    print("finished getting detections")
                    writer.writerow(tmp)
                    _t['misc'].toc()
                    print('finished %d' % (image_id))
                    if (count % 100) == 0:
